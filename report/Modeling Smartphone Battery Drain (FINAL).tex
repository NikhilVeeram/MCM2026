...
For OLED displays, specifically, $\gamma(t) \approx 0.3$ in Dark Mode versus $\gamma(t) \approx 0.9$ in Light Mode, validating the 40\% savings observed in \cite{5}.
...
# Calibrated Physical Constants (Derived from Source Code)
Q_CAP = 5000 * 3.6    # 18,000 Coulombs (5000mAh)
R_REF = 0.12          # Internal Resistance (Ohms)
V_NOM = 3.8           # Nominal Voltage (Volts)
T_AMB = 22.0          # Ambient Temperature (Celsius)

class MCMHighFidelityModel:
    def get_ocv(self, soc):
        """Empirical Li-Ion chemistry curve [Linear Approximation]"""
        # Matches App.tsx: const voc = 3.45 + 0.75 * this.soc;
        return 3.45 + 0.75 * soc
    ...
    def get_p_total(self, t, s, temp, p):
        ...
        # Matches App.tsx: 12.0 * (0.8 * (util)^3)
        p_cpu = 12.0 * (0.8 * (p['u']**3)) + (p['p_app'] * 0.45)
        
        # 2. Display Power (OLED Non-linear Scaling)
        p_disp = (1.5 * (p['b']**1.8) * p['apr'] + 0.1)
        
        # 3. Network Signal (SNR Penalty & Tail Energy)
        # Matches App.tsx: 10^((-signal - 80) / 25)
        delta = 10**((-p['signal'] - 80) / 25.0)
        p_net = (min(3.0, (0.2 + delta * 2.5)) if p['net'] else 0.02)
        
        p_total = (p_cpu + p_disp + p_net + 0.1) * u_mult
    ...
    def system_dynamics(self, y, t, p):
        ...
        s, vp1, vp2, temp = y
        # Calibrated Thermal Internal Resistance
        r0_t = R_REF * (1 + 0.003 * (temp - 25.0)) 
        
        p_total = self.get_p_total(t, s, temp, p)
        
        # Current-dependent efficiency logic
        voc = self.get_ocv(s)
        ...
        # Note: App uses accelerated thermal dyn (factor ~40x)
        ds = - (i_load) / Q_CAP if s > 0 else 0
        dvp1 = - (vp1 / 30.0) + (i_load / 1200.0)
        dvp2 = - (vp2 / 300.0) + (i_load / 6000.0)
        dtemp = ((i_load**2 * r0_t * 4.0) + p_total*0.1 - 0.18*(temp - T_AMB)) / 10.0

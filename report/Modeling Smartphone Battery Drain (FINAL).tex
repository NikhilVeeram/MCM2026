...
To render the complex electrochemical systems in a mathematically defined way, we make the following modeling assumptions based on our initial ideation and extensive literature review:
...
\begin{lstlisting}[language=Python, caption=Calibrated ODE Solver and Dynamics]
    def system_dynamics(self, y, t, p):
        s, vp1, vp2, temp = y
        # Calibrated Thermal Internal Resistance [Eq 3]
        r0_t = 0.15 * (1 + 0.003 * (temp - 25.0))
...
\begin{lstlisting}[language=Python, caption=User Scenarios and Simulation Suite]
def run_calibrated_suite():
    model = MCMHighFidelityModel()
    t = np.linspace(0, 24 * 3600, 8000) 
    y0 = [0.95, 0, 0, 22.0] # 95% SOC, Ambient Temp

    # Define profile parameters optimized for iPhone 17 Pro / Pixel 8
    scenarios = [
        ("Ultra Gaming (5G)", {'f': 1.0, 'u': 0.98, 'p_app': 5.0, 
                               'b': 1.0, 'apr': 0.95, 'signal': -110, 'net': True}),
        ("Social Media", {'f': 0.4, 'u': 0.4, 'p_app': 0.6, 
                          'b': 0.7, 'apr': 0.6, 'signal': -85, 'net': True}),
        ("Deep Standby", {'f': 0.05, 'u': 0.02, 'p_app': 0.0, 
                          'b': 0.0, 'apr': 0.0, 'signal': -90, 'net': False})
    ]

    for name, config in scenarios:
        res = odeint(model.system_dynamics, y0, t, args=(config,))
        # Process and validate deaths index...
\end{lstlisting}
\endgroup

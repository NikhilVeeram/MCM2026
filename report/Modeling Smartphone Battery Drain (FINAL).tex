...
\begin{figure}[h]
    \centering
    \fbox{\parbox[c][4cm]{0.95\textwidth}{\centering \vspace{1.5cm} \textbf{Placeholder: iOS App UI Screenshot} \\ (To be inserted: Real-time monitoring view) \vspace{1.5cm}}}
    \caption{Proposed iOS App Interface for Real-Time Battery Monitoring.}
    \label{fig:ios_app_placeholder}
\end{figure}
...
% =================================================================================
\section{Appendix A: Core Simulation Logic}
This appendix presents the Python implementation of the Continuous-Time Kinetic Battery Model (CT-KBM) derived in Sections 5 and 6. The code, encapsulated in the \texttt{MCMHighFidelityModel} class, numerically solves the system of differential algebraic equations (DAE) to predict battery state of charge (SOC), voltage, and temperature over time. The model is built using the \texttt{scipy.integrate.odeint} library, which utilizes the LSODA solver for stiff differential systems.

\subsection*{A.1 Physical Constants and Calibration}

The simulation is initialized with physical parameters calibrated from the NASA PCoE dataset \cite{8} and the specific power coefficients derived for the Google Pixel 8 and iPhone 17 Pro case studies. These constants define the baseline thermal and electrical characteristics of the device before any user input is applied.

\begin{lstlisting}[language=Python, caption={Model Constants and Calibration}]
import numpy as np
...
# ==========================================
Q_CAP = 5.0 * 3600    # Battery Capacity (Coulombs) -> 5000mAh
R_REF = 0.15          # Internal Ohmic Resistance (Ohms) at 25C
ALPHA_THERM = 0.003   # Resistance Thermal Coefficient (per deg C)
T_REF = 25.0          # Reference Temperature (deg C)
C_TH = 150.0          # Thermal Mass (J/K) for Heat Dissipation
H_CONV = 0.22         # Convective Heat Transfer Coeff (W/K)
\end{lstlisting}

\subsection*{A.2 Modeling Power Consumption}

The \texttt{get\_p\_total} method calculates the instantaneous power demand $P_{total}(t)$ by summing the contributions of the CPU, display, and network components. This function implements the algebraic constraints defined in Section 6, ensuring that software inputs are properly scaled by hardware physical laws.

\textbf{CPU Power Dynamics:}
Lines 33-36 implement Equation (\ref{eq:cpu_power}), modeling the convex relationship between frequency and power. The term \texttt{(0.7 * ... **2)} represents the dynamic scaling factor $\alpha V^2 f$, while \texttt{p['p\_app']} adds the specific overhead from the active app cluster.

\textbf{Display and Network Physics:}
Lines 39-43 implement the OLED power law (Equation \ref{eq:display_power}) and the signal-dependent network penalty (Equations \ref{eq:network_power} and \ref{eq:snr_penalty}). Note the exponential penalty \texttt{delta} driven by signal strength variance, which forces the model to account for "dead zones" where power consumption spikes.

\begin{lstlisting}[language=Python, caption={Hardware Power Aggregation Logic}]
    def get_p_total(self, t, s, temp, p):
        """
        Calculates P_total(t) [Section 6.3]
        """
        u_mult = p.get('user_scale', 1.0) 
        
        # 1. CPU Power [Eq. \ref{eq:cpu_power}]
        # Convex DVFS Model: P ~ f^3 * U + P_static
        # Calibrated to 12.0W Peak for Performance Cores
        p_cpu_base = 12.0 * (0.7 * (0.8 + 0.3*p['f'])**2 * p['f'] * p['u']) \
                     + (p['p_app'] * 0.45)
        p_cpu = p_cpu_base * u_mult

        # 2. Display Power (OLED) [Eq. \ref{eq:display_power}]
        # Scaled by Brightness and Active Pixel Ratio (APR)
        p_disp = (1.5 * (p['b']**1.6) * p['apr'] + 0.03) * u_mult
        
        # 3. Network Signal [Eq. \ref{eq:network_power}]
        # Path Loss Penalty [Eq. \ref{eq:snr_penalty}]
        delta = 10**((-p['signal'] - 80) / 20)
        p_net = (min(3.0, (delta * 0.25)) if p['net'] else 0.02) * u_mult
        
        p_total = p_cpu + p_disp + p_net + 0.04
        
        # 4. Thermal Throttling Feedback [Eq. \ref{eq:thermal_throttle}]
        # Negative feedback loop when Temp > 40C
        # Simulates OS-level protection mechanisms
        if temp > 40.0:
...
\subsection*{A.3 Solving the Differential System}

The core dynamics are resolved in \texttt{system\_dynamics}, which calculates the derivatives for the SOC, polarization voltages, and temperature at each time step. This method numerically solves the coupled system described by Equations (\ref{eq:gov_soc})-(\ref{eq:gov_temp}).

\textbf{Voltage and Current Constraints:}
Lines 70-78 solve the quadratic voltage constraint (Equation \ref{eq:voltage_term}) to determine $I_{load}$. This couples the electrical state with the power demand: as voltage sags, current must increase to maintain constant power, creating a runaway effect near 0\% SOC.

\textbf{State Derivatives:}
Lines 83-90 compute the rate of change for the state variables. Note the inclusion of \texttt{eta\_i} (Equation \ref{eq:efficiency_refined}) in the SOC derivative to account for Peukert-like inefficiency at high loads.

\begin{lstlisting}[language=Python, caption={Differential System Solver}]
    def system_dynamics(self, y, t, p):
        s, vp1, vp2, temp = y
        s = max(0, s) 
        
        # Update Internal Resistance [T-dependence]
        r0_t = R_REF * (1 + ALPHA_THERM * (temp - T_REF))
        p_total = self.get_p_total(t, s, temp, p)
        
        # Load Current Calculation (KVL) [Eq. \ref{eq:voltage_term}]
        voc = self.get_ocv(s)
        v_diff = voc - vp1 - vp2
        
        # Solve Quadratic for I_load: P = V_term * I
        disc = v_diff**2 - 4 * r0_t * p_total
        
        if disc < 0:
            i_load = p_total / 3.0 # Collapse safeguard
        else:
            i_load = (v_diff - np.sqrt(disc)) / (2 * r0_t)
        
        # Coulombic Efficiency [Eq. \ref{eq:efficiency_refined}]
        eta_i = 0.99 - 0.015 * (p_total / 8.0) 

        # 1. SOC Derivative [Eq. \ref{eq:gov_soc}]
        ds = - (i_load) / (Q_CAP * eta_i) if s > 0 else 0
        
        # 2. Polarization Dynamics [Eq. \ref{eq:gov_vp1}, \ref{eq:gov_vp2}]
        dvp1 = - (vp1 / 30.0) + (i_load / 1500.0)
        dvp2 = - (vp2 / 300.0) + (i_load / 8000.0)
        
        # 3. Thermal Dynamics [Eq. \ref{eq:gov_temp}]
...
\subsection*{A.4 Execution and State Estimation}

The system is integrated using \texttt{scipy.integrate.odeint}, allowing us to simulate different user scenarios by passing distinct configuration dictionaries. The resulting time-series data is then plotted to generate the curves seen in Figures \ref{fig:scenario_grid} and \ref{fig:power_breakdown}.

\begin{lstlisting}[language=Python, caption={Integration of User Scenarios}]
    # Differential Equation Solver
    # Solves y' = f(y, t) for the setup scenarios
    # args=(config,) passes the user profile (Gaming, Eco, etc.)
    res = odeint(model.system_dynamics, y0, t, args=(config,))
    
    # Extract results state vector
    soc = res[:, 0] * 100         # SOC %
    temp = res[:, 3]              # Temperature (C)
\end{lstlisting}
